...
async function getAll({ customerId, ...options } = {}) {
  ...
  try {
    const rows = await db.StripeUser.findAll({
...
async function getById({ id, customerId, ...options }) {
  const t = await beginTransactionWithCustomerContext(customerId);
  try {
    const row = await db.StripeUser.findOne({
      where: { id },
      ...options,
      transaction: t,
    });
    if (!row) throw { status: 404, message: "Stripe user not found" };
    await t.commit();
...
async function create({ data, customerId, ...options }) {
  const t = await beginTransactionWithCustomerContext(customerId);
  try {
    const result = await db.StripeUser.create(
      {
        ...data,
        customerId,
...
async function update({ id, data, customerId, userId, ...options }) {
  ...
  try {
    ...
    const result = await db.StripeUser.findOne({
      where: { id },
      ...options,
      transaction: t,
    });
    if (!result) throw { status: 404, message: "Stripe user not found" };
    await t.commit();
    return result.get({ plain: true });
  } catch (error) {
    await t.rollback();
...
async function patch({
  ...
  try {
    const [count, [updated]] = await db.StripeUser.update(
      { ...data, updatedBy: userId || data.updatedBy },
    ...
    if (count === 0)
      throw {
        status: 404,
        message: "Stripe user not found or update blocked by RLS.",
      };
...
async function _delete({ id, customerId, userId, ...options }) {
  const t = await beginTransactionWithCustomerContext(customerId);
  try {
    const count = await db.StripeUser.destroy({
      where: { id },
      ...options,
      transaction: t,
    });
    if (count === 0)
      throw {
        status: 404,
        message: "Stripe user not found or delete blocked by RLS.",
      };
    await t.commit();
  } catch (error) {
    await t.rollback();
