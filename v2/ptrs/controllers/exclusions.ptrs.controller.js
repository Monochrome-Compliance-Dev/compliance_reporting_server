...
async function exclusionsSummary(req, res, next) {
  const customerId = req.effectiveCustomerId;
  const userId = req.auth?.id;
  const ip = req.ip;
  const device = req.headers["user-agent"];
  ...
  try {
    ...
    const out = await exclusionsService.getExclusionsSummary({
      customerId,
      ptrsId,
      profileId,
    });
    ...
    await auditService.logEvent({
      ...
      ip,
      device,
      action: "PtrsV2ExclusionsSummary",
      entity: "PtrsUpload",
      entityId: ptrsId,
      details: {
        totalExcludedRows: out?.totalExcludedRows ?? 0,
      },
    });

    return res.status(200).json({ status: "success", data: out });
  } catch (error) {
    logger.logEvent("error", "Error loading PTRS v2 exclusions summary", {
      action: "PtrsV2ExclusionsSummary",
      ptrsId,
      customerId,
      userId,
