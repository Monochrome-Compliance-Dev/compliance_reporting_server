...
async function applyCrossRowRulesSql({
  ...
  for (const rule of enabled) {
    ...
    const transformedTargetValueSql = `ROUND((${mathSql})::numeric, ${dp})`;
    const targetTypedColumn =
      getStageColumnForCanonicalField(targetAmountField);
    const targetTypedAssignmentSql = targetTypedColumn
      ? `"${targetTypedColumn}" = ${transformedTargetValueSql},`
      : "";
    ...
    let targetDataSql = `
      jsonb_set(
        ...
        '{${targetAmountField}}',
        to_jsonb(${transformedTargetValueSql}),
        true
      )
    `;
